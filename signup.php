        ...
        try {
			...
			if($stmt->fetchColumn()==0){
	            if(preg_match("/^=.+$/",$username)){
		            ...
		            $stmt = $pdo->prepare("INSERT INTO userData(name, password, accesslevel) VALUES (?, ?, ?)");

		            $stmt->execute(array($username, password_hash($password, PASSWORD_DEFAULT), 'F'));  // パスワードのハッシュ化を行う（今回は文字列のみなのでbindValue(変数の内容が変わらない)を使用せず、直接excuteに渡しても問題ない）
